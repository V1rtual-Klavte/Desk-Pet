...
export const memoryContract: ModuleContract = {
  ...
  sourceHash: "ee737f46ed024ae06be20a4565712e22e9da2a45d6fa0c0d4eb87486a65b60b4",
  coverage: [
    ...
    { id: "mm-11", feature: "PromptSnapshot", description: "三档快照（transform_context/provider_payload/provider_usage）可关联 request/turn/run；快照带请求归属（request.purpose/step/attempt）、参数（systemPromptHash 三档可比；payloadHash 与 requestParams 由 before_payload 从 Provider payload 采集，只落在 provider_payload 档、取不到就不写，不粘到同回合其它档）、计划与能力（plan/冻结的 capabilities.safetyMode 与逐请求累积的 toolDecisions）、槽代际（generation）、内核淘汰（budgetDrops）与换代身份（compaction.count、本分支最近一条压缩条目地址、摘要 hash）；一次性压缩请求以 one-shot:compaction 身份另立 payload+usage 两档条目，不写归属错误的主回合 payload（摘要正文只留 hash）；派生 rewrite 只保存 hash 与压缩条目地址；每条工具条目的 policyHash 取完整策略指纹 toolPolicyHash(tool)（工具身份与 permission/execution/context 各维度），策略任一维度变化都会改变快照身份；投递输入的 agentMessages id 用其 deskpetEventId（无身份的用位置号回退），请求与输入的关联可按身份核对", why: "上下文审计不能泄露原始 prompt、覆盖用户原文或把估算 token 当成实际 usage；「这是哪次请求、带什么参数、第几代槽」必须能从快照回答", depth: "deep", scenarios: ["memory-prompt-snapshot", "memory-snapshot-identity"] },
