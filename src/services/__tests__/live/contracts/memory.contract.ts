...
export const memoryContract: ModuleContract = {
  ...
  sourceHash: "182c0ece894d7090643fd9128239286b04661e5b050db8f3e0639cd56daa836b",
  coverage: [
    ...
    { id: "mm-18", feature: "消息事件单次写入", description: "一次真实回合后同一句话只有一条 deskpet-event；新回合不得同时写 legacy deskpet-turn", why: "两个协议双写会让重载各重放一次，造成重复消息与翻倍轮数", depth: "deep", scenarios: ["memory-single-message-write"] },
    { id: "mm-19", feature: "压缩检查点提交与恢复", description: "fake provider 生成结构化摘要后，以 session version/CAS 提交覆盖边界；生成中取消、非法摘要或版本变化不写入；手动切标签仍提交捕获目标，多次重载保留 hash 链、完整正文和工具 round", why: "摘要只能替换后续请求视图，不能删除会话真相源或让旧异步结果覆盖新事件", depth: "deep", scenarios: ["memory-compaction-checkpoint"] },
  ],
