...
export const memoryContract: ModuleContract = {
  ...
  coverage: [
    ...
    { id: "mm-06", feature: "本地整理 Consolidate", description: "MemoryService.consolidate() 只做本地去重，不请求 LLM", why: "后台维护不能抢占首答或未经授权提取长期事实", depth: "shallow", scenarios: ["memory-consolidate"] },
    ...
    { id: "mm-11", feature: "PromptSnapshot", description: "三档快照（transform_context/provider_payload/provider_usage）可关联 request/turn/run；快照带请求归属（request.purpose/step/attempt）、参数（systemPromptHash 是整段 systemPrompt 的 hash：同一请求的三档必须一致，跨请求会随动态层注入的当前时间每分钟漂移 —— 它只证明「同一次请求的三档没被换过」，不是配置指纹；payloadHash 与 requestParams 由 before_payload 从 Provider payload 采集，只落在 provider_payload 档、取不到就不写，不粘到同回合其它档）、计划与能力（plan/冻结的 capabilities.safetyMode 与逐请求累积的 toolDecisions）、槽代际（generation）、内核淘汰（budgetDrops）与换代身份（compaction.count、本分支最近一条压缩条目地址、摘要 hash）；一次性压缩请求以 one-shot:compaction 身份另立 payload+usage 两档条目，不写归属错误的主回合 payload（摘要正文只留 hash）；派生 rewrite 只保存 hash 与压缩条目地址；每条工具条目的 policyHash 取 toolPolicyHash(tool)，即策略指纹的 SHA-256：指纹含工具身份（id/source/sourceId）、策略版本、defaultDecision、effect/isolation/replay、resultProjection/historyCompaction、safetyLevel 与 actionCategory 共 12 项（原 `mode` 维度已随模式删除消失；策略版本已递增到 2，裁决语义变更前后写入的条目 hash 不同 —— 会话内 5 分钟的同参授权也随之不复用），这 12 项任一变即改变该工具在快照里的身份；投递输入的 agentMessages id 用其 deskpetEventId（无身份的用位置号回退），请求与输入的关联可按身份核对", why: "上下文审计不能泄露原始 prompt、覆盖用户原文或把估算 token 当成实际 usage；「这是哪次请求、带什么参数、第几代槽」必须能从快照回答", depth: "deep", scenarios: ["memory-prompt-snapshot", "memory-snapshot-identity"] },
    ...
    { id: "mm-15", feature: "Plan checkpoint 恢复", description: "运行中 Plan 从会话条目 deskpet.plan_checkpoint 按事件级证据折迭恢复，产出可处置的 paused 计划（只读步骤回 pending；有 tool_start 无 tool_end 的效果工具进 unknown_side_effect，末事件是 tool_end 的取末事件回 pending）；plan 的唯一持久形态是 schemaVersion 2，旧格式条目（含存量 1）按不可恢复跳过、不做迁移；继续/丢弃由用户显式触发，未知副作用不自动重放，恢复动作可条目回读", why: "进程重启后不能重复执行没有完成凭证的外部操作", depth: "deep", scenarios: ["memory-plan-resume"] },
    { id: "mm-16", feature: "上下文内核（纯预算）", description: "buildPromptBlocks 只做预算：窗口预算闭合（硬输入上限 + 输出预留 + 协议开销）、按层排序与拼接；核心块（静态人格/CANDY/工具协议/工具 schema/动态运行时/会话摘要）放不进硬上限时显式 ContextBudgetError、绝不截块内文字，可选块整块淘汰并记入 budgetDrops；分配账目按层给出 requested/assigned/used（transcript 行保留但不再有份额与借用字段，没有淘汰的层不写 dropped: 0）；完整静态前缀不随变量变化、工具 schema 完整计入输入预算；含会话正文的完整请求视图由托管钩子按同一硬输入上限核查（mm-22 的溢出恢复依赖它）", why: "Provider 输入需要可预测且可审计地限制在上下文窗口内，且不能静默丢失指令或未压缩历史", depth: "deep", scenarios: ["memory-context-budget"] },
    ...
    { id: "mm-29", feature: "动态提示与思考强度文案", description: "聊天动态提示由 composeDynamicPrompt + CHAT_THINKING_HINTS 唯一拼接：变量池正文 + 强度后缀 + 末尾定长的当前时间（`[当前时间] YYYY-MM-DD HH:mm 周X`，26 字符、分钟精度，排在最后以免顶掉池正文的前缀缓存收益），时间片段落在 dynamic 层的动态运行时块、不进静态前缀；一次性调用不经过这条拼接，非推理模型 + low 档时的兜底提示是 ONE_SHOT_LOW_EFFORT_HINT；两者刻意不同。注：本覆盖点的 unit 场景仍按「auto/medium 档原样返回池正文」断言，时间拼接落地后该断言失效，语义重写归 T19", why: "文案散落三处时改一处就分叉，且没有任何断言拦它；时间注入不进 static 前缀是「静态成分不变」的判据", depth: "shallow", scenarios: ["memory-prompt-composition"] },
    ...
    { id: "mm-31", feature: "召回文本按估算器口径裁剪", description: "MemoryProvider 召回正文按 estimateContextTokens 口径裁剪：超预算时截断并显式标记，不再用 4 字符 = 1 token 的通吃常数", why: "中文下 4 字符常数会让实际用量达声明值约 4 倍，是 P6 的预算穿透点", depth: "shallow", scenarios: ["memory-recall-token-budget"] },
  ],
