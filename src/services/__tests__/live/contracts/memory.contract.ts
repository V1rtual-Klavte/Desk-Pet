...
export const memoryContract: ModuleContract = {
  ...
  sourceHash: "064b4a39cf3b2d0cdac20fe40685f9d1a0521942326d5fb632304d9182f8dde1",
  coverage: [
    ...
    { id: "mm-11", feature: "PromptSnapshot", description: "transformContext 后与 Provider payload 双快照可关联 request/turn/run；派生 rewrite 只保存 hash；每条工具条目的 policyHash 取完整策略指纹 toolPolicyHash(tool)（工具身份与 permission/execution/context 各维度），策略任一维度变化都会改变快照身份", why: "上下文审计不能泄露原始 prompt、覆盖用户原文或把估算 token 当成实际 usage", depth: "deep", scenarios: ["memory-prompt-snapshot"] },
