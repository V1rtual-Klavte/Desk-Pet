...
export const plannerContract: ModuleContract = {
  ...
  coverage: [
    ...
    { id: "pl-06", feature: "executePlan 步骤执行", description: "按计划逐步执行：步骤顺序与 onStepStart/onStepDone 回调顺序都是计划顺序，逐条记账成功/失败，全成功才判 overallSuccess；步骤未限定 allowedTools 时经 onStepNotice 如实报告工具面放大（宿主按「除派生型工具外的全部已注册工具」写进度与系统消息，派生型工具在子代理入口还会再被剥掉），指定了不存在的工具、或指定了到不了子代理的派生型工具（isolation=delegate，现只有 agent_spawn）走同一条硬失败：不开工、步骤判失败、产出带工具名的错误、不是取消归宿。两种「解析不到」都由本覆盖点的 unit 场景断言 —— 派生型工具那段用 agent_spawn 这个名字（getToolByName 按 AI 调用的函数名解析，不是工具 id）：恰一条带工具名的 missing_tools 通知、零 Provider 请求、overallSuccess=false、产出 error 带工具名、reply 为空且 toolCallsMade=0（快照归属那一半仍未由 unit 场景断言，production 证据见 mm-11）", why: "Plan 执行闭环，且步骤的权限面变化不能只留在日志里", depth: "deep", scenarios: ["plan-execute-loop"] },
    ...
    { id: "pl-09", feature: "生产入口的计划确认与进度", description: "生产入口跑完整计划闭环：确认视图与进度事件的步数都是截断后（maxSteps 生效后）的步数，终态同时落在 checkpoint 条目（terminal 快照 done、全量步骤基线两步都 done）与终态事件（done）上，步骤结果条目恰为计划步数；确认经会话键控的测试通道确定性应答（mode=auto）并带会话身份。计划段由 `planConfig.enabled` 单独把守（模式已删除、该开关出厂为 true；Live 基线把它钉在 false，本场景在 setup 里显式打开）", why: "计划入口此前只有判定与解析的 unit 覆盖，确认视图、进度 total 与终态证据这些真正的运行时接线没有任何生产入口证据", depth: "deep", scenarios: ["plan-production-loop"] },
