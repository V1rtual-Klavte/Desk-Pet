...
export const toolExecutionContract: ModuleContract = {
  ...
  coverage: [
    ...
    { id: "te-24", feature: "MCP 每服务器开关", description: "MCP 的控制面只有每服务器的 enabled（缺省即启用，只有显式 false 才算关闭），总闸 tools.mcp.enabled 已随决策 8 删除：「MCP 是否生效」与「本轮该借用哪些服务器」共用同一份 enabledMcpServerNames()（computeMcpEnabled() 只是它的派生值：至少一个服务器启用），能力准备按它逐个借用、全关时无人可借也就不连接任何服务器。随包默认把内置清单（`tools.mcp.builtin`）里的五个服务器全部设为 enabled: false，保持删总闸之前实际生效的状态（删总闸而不改默认值会让出厂行为悄悄翻成「自动挂载对 / 的文件系统访问」）。本点覆盖运行期口径与注册结果；开关的读取定义点在 config.ts 的类型化 getter（按配置同步清单维护）", why: "总闸删除后若没有逐项开关，出厂默认会从「全关」翻成「全开」；「全关 = 不连接」是这条翻转载荷唯一的可观测结论", depth: "shallow", scenarios: ["tool-mcp-server-toggle"] },
  ],
