...
export const toolExecutionContract: ModuleContract = {
  ...
  sourceHash: "0f27accffb194603159cdcbd4557977fc6c76d19c43893687a3cc4a655f6a0d9",
  coverage: [
    ...
    { id: "te-16", feature: "纯读并行与效果互斥", description: "shared_read 按 Rust 应用级许可的有界共享额度并发（两个只读真的重叠），共享读上限由 `ai.loop.maxParallelTools` 下发（范围 1–8：上限 1 只放一个读、提高上限唤醒排队读、越界下发被拒绝且不改变生效值、降低上限不撤销在飞许可），exclusive_effect 与进行中的读写互斥：等待发生在许可队列（queued 可观测）、独占期间开不出新读、读不插队到排队中的独占之前、取消排队中的独占会重新放行后面的读、执行结束才释放额度且没有泄漏", why: "并行不能牺牲互斥：写类与其他执行必须串行，等待可取消且不让读饿死写，额度必须归还干净，上限变更不能留下越界或未生效的额度语义", depth: "deep", scenarios: ["tool-execution-permit"] },
