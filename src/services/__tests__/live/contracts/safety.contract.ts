...
export const safetyContract: ModuleContract = {
  module: "safety",
  sourceFiles: ["src/services/engine/pi/runtime.ts", "src/services/safety/checker.ts", "src/services/safety/permission.ts", "src/services/safety/confirm.ts", "src/services/tool/types.ts", "src/services/tool/mcp/client.ts"],
  generatedAt: "2026-09-17",
  sourceHash: "c01575ca036f5a31e884455187f3d0dde80adc86fa0f6c378eac39289fcecddb",
  coverage: [
    ...
    { id: "sf-11", feature: "MCP passthrough 终裁", description: "MCP 适配器只能返回 passthrough；PermissionKernel 必须将其收敛为 allow、ask 或 deny，executor 不得看到 passthrough", why: "发现远端工具不等于默认信任", depth: "deep", scenarios: ["permission-passthrough-final"] },
    { id: "sf-12", feature: "PermissionKernel deny-first", description: "不可放宽的 NOWAY 硬拒绝优先于工具或来源的 allow，独立 ask 不被普通 allow 吞掉", why: "权限组合必须 fail-closed", depth: "deep", scenarios: ["permission-deny-first"] },
    ...
    { id: "sf-14", feature: "精确会话授权", description: "allow_session 仅复用相同 session、generation、tool、参数与策略的未过期授权", why: "一次确认不能扩大到其他输入或回合", depth: "deep", scenarios: ["permission-session-grant"] },
