...
export const personalityCardContract: ModuleContract = {
  ...
  coverage: [
    ...
    { id: "pc-09", feature: "阶段文案链路", description: "用户可见的阶段与兜底文案只有 Card 一个来源，四个 getter 各自到达取用点：工具类别由 ToolDef.actionCategory 经 actionCategoryOf 解析（对活动注册表的一次按名查找，未知名字与已释放的 MCP 工具同形回 _default —— 技能不再是工具，不进这张表），getStagePrompt 返回当前 Card 的 executing/done/blocked 文案（缺类别/缺缓存回退 _default）；getSimpleStage 覆盖 thinking/planning/error/retry 标量；getCommandReply 覆盖 commands 段的全部 13 个键（含 `/skill` 的 skillStarted/skillUnknown/skillEmpty/skillDisabled 四个终态键；技能名与「未知 / 无正文 / 被关闭」的诊断事实由命令层作中性明细附在 Card 终态句之后，不写进 key）；getFallbackReply 覆盖 fallbacks 段全部兜底键（数组型按元素命中）。逐 key 比对探针 Card 与中性常量的差异，Card 完全不可用时全部回非空常量 —— 空串会让 UI 静默显示空框", why: "Card 定制的过程语气是产品原则，链路静默变空或退回中性常量时界面看不出差别，全套 Live Test 不容易发现", depth: "shallow", scenarios: ["stage-prompt-link"] },
