...
const scene: SceneDef = {
  ...
  turns: [{ index: 1, description: "验证 Hook 生命周期错误码", userText: "检查 Hook 门禁。", checks: [{ type: "expectSafety", run: async () => {
    ...
    bus.register({ id: "timeout", mode: "blocking", deadlineMs: 1, handle: async () => new Promise(() => {}) })
    const timeout = await bus.emit({ hookId: "timeout", name: "timeout", sessionId: "s", turnId: "t", runId: "r", taint: "trusted_user", payload: {} })
    if (timeout.decision !== "block" || timeout.errorCode !== "hook_timeout") throw new Error("Hook 超时错误码不稳定")
    bus.clear()
    bus.register({ id: "reentrant", mode: "blocking", handle: async event => bus.emit(event) })
    const reentrant = await bus.emit({ hookId: "reentrant", name: "reentrant", sessionId: "s", turnId: "t", runId: "r", taint: "trusted_user", payload: {} })
