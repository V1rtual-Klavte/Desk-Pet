 ...
 *
 * 未覆盖（报告登记）：Pi read 工具自身的接线（`imageProcessor` 选项是否还传着）与它在
 * 条目里的图片 part —— 场景无法把二进制夹具写进数据根（`TauriExecutionEnv.writeFile` 拒绝
 * Uint8Array，Rust `file_write` 只收字符串）。
 */
...
function bmpFixture(width: number, height: number): Uint8Array {
