...
async function initFromStorage() {
  try {
    console.log("[LayerEditor] 开始初始化...");
    await initProfiles();
    const p = getActiveProfile();
    profile.value = p;
    if (!p) {
      console.error("[LayerEditor] getActiveProfile() 返回 null！");
      return;
    }
    ...
    ready.value = true;
    console.log("[LayerEditor] ✅ 就绪");
  } catch (err) {
    console.error("[LayerEditor] init 异常:", err);
  }
}

function refreshLayerUrl(i: number) {
  const p = profile.value;
  if (!p) { layers.value[i].url = null; return; }
...
function onImgError(i: number, e: Event) {
  ...
  console.warn(`[LayerEditor] L${i} 加载失败: ${img.src}`);
...
// ── ☆ 在 canvas 上监听 pointermove/pointerup，用 setPointerCapture 确保可靠 ──
function onPointerDown(index: number, e: PointerEvent) {
  console.log("[LayerEditor] pointerdown L" + index + " locked=" + layers.value[index].config.locked);
...
onMounted(async () => {
  ...
  try { await win.setTitle(`🎨 图层编辑器 - ${profile.value?.meta.name || "糖糖桌宠"}`); } catch {}
  // ☆ 监听可用空间自动调整画布尺寸
  if (canvasWrap.value) {
