...
- **会话运行槽与队列化入口** — 聊天消息先将 queued 事实事件原子写入 sessions，再进入 Pi；SessionTurnStore 通过版本/CAS 记录 queued 到 done/failed，按 sessionId 隔离的 AgentSlot 持有当前 Agent 和 generation，当前回合结束后自动消费同会话 pending 消息
