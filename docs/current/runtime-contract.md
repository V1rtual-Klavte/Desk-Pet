...
忙碌输入的持久状态由 lane inbox 承担：投递即 commit 进会话文件；被消费即移出并进入正文；未消费项在停止/中断后归还，不会虚假 accepted。单项撤回经 `cancelQueued` 返回 `cancelled / already_consumed / not_found`；排队项另有只读视图（`listQueuedInputs`）供 UI/场景使用，UI 不再自建队列状态。[双模式方案](../plans/active/Pi运行时与工具协议建设方案.md#3-steer-与-follow-up-双模式)的显式投递意图选择、排队视图与单项撤回已落地（PI-1 的 UI 范围）；更细的逐项证据状态（request_prepared 等）尚未实施。
...
0.85.1 的 AgentHarness、JsonlSessionRepo 与压缩调度已接入为运行内核（§8 迁移已实施并通过 2026-09-18 集中验证，证据见[执行手册](../plans/active/记忆系统重构执行手册.md#已有验证证据)，协议正文见[归档基线](../history/implementation/AgentHarness迁移方案-2026-09-18基线.md)）；插话双模式的输入意图选择、排队视图与单项撤回（PI-1 的 UI 范围）与 usage purpose 单列（PI-4 剩余项）已落地，工具策略/只读并行（PI-2）尚未实施，见[Pi 建设方案](../plans/active/Pi运行时与工具协议建设方案.md)。
