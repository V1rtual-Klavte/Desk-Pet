...
复杂任务可通过 Plan 拆解步骤。步骤按顺序串行执行，不再有依赖/并行标注。计划确认时可以选择「全部执行」，也可以选择「逐步确认」：逐步确认在每一步开跑前停下来等你决定继续还是中止，中止后剩余步骤保持未执行；某一步失败时也停下来问，不会自动往下跑。步骤与工具状态保留检查点，恢复时不会自动重试结果未知的外部副作用。计划被取消、超时或按你的选择中止时会停在当前步骤、剩余步骤不执行，聊天区给出说明；这类计划与恢复产出的暂停计划都不会自动重放或自动续跑，须由你处置后才继续。计划若因记录写入失败而无法留痕，会如实提示「计划执行记录写入失败」，不假装计划没跑过。当前验证范围和未完成体验项见[未完成工作与已知缺口](plans/active/未完成工作与已知缺口.md)，不把具备 Plan 接口等同于所有任务均可无人值守完成。
