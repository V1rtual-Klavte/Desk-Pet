...
普通消息先保存再交给模型处理，运行中仍可继续发送。当前已有插话与后续队列接线，逐条消费确认及回合收尾边界仍待完善，不能把“已排队”视为“已处理”。切换标签后，原会话的后台结果仍归原会话；失败会保留失败状态，不因展示了角色化兜底文案而算作成功。

目标体验是默认“插话”，也可选择“稍后继续”，并可查看排队状态或停止当前处理；目前尚无完整的双模式选择界面。插话在当前响应及工具批次结束后参与后续回答，不表示撤销已经发生的操作，见[双模式建设方案](plans/active/Pi运行时与工具协议建设方案.md#3-steer-与-follow-up-双模式)。
...
流式正文已接入：回复生成期间按增量展示正文（不含思考内容，运行数据标签不进瞬时展示）；下一步完善双模式选择界面、逐条处理状态与可控的只读工具并行，见[Pi 建设方案](plans/active/Pi运行时与工具协议建设方案.md)，当前工具仍保持串行。
