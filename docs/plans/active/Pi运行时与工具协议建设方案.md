...
status: section8_verified_rest_pending
updated_at: 2026-09-18
code_baseline: b2a1e41
...
本文落实 Pi 能力审查与用户补充：**steer 是聊天感的核心，follow-up 同时保留；只读工具可以并行，写入等有顺序要求的工具串行；每个工具明确声明权限、输出压缩与执行策略。** **§8 的 AgentHarness 迁移（H-1–H-4）已实施并通过 2026-09-18 集中验证**，协议正文归档为 [AgentHarness 迁移方案基线](../../history/implementation/AgentHarness迁移方案-2026-09-18基线.md)，本页 §8 只留索引；**§3–§5 的 UI、配置字段与工具策略等目标仍未实现**，旧测试记录不能证明这些目标已通过。

当前事实分别维护在[运行时契约](../../current/runtime-contract.md)、[工具系统](../../current/tool-system.md)、[记忆与压缩](../../current/memory.md)。本文是目标协议的主要维护位置，执行进度只记录在[执行手册](记忆系统重构执行手册.md#当前检查点)。不要求每次任务通读本文：插话读 §3，工具读 §4–5，Pi 接线读 §6，压缩读 §7，Harness 迁移历史读归档基线，实施与配置读 §9–10。
...
8. 采用 AgentHarness 替换宿主运行状态机（§8，已完成并通过集中验证）：存储走 JsonlSessionRepo；旧会话数据按测试数据弃用，不迁移、不双写。宿主自建的队列、确认与压缩调度由 Harness 承接；PI-2 工具策略保持独立实施。
...
上述竞态是静态审查发现的风险，须通过 §10 场景验证；不能写成已复现、已修复或已通过。该表是迁移前基线的缺口盘点；H-1–H-4 已按 §8 落地并通过集中验证（协议见[归档基线](../../history/implementation/AgentHarness迁移方案-2026-09-18基线.md)），表中其余行描述的是迁移前的宿主实现。
...
本节描述的是**要求**，不是实现位置：AgentSlot 已随 H-4 退役，接收窗口、代际绑定与收尾职责现由 Harness 的 lane / operation 生命周期承担（[归档基线](../../history/implementation/AgentHarness迁移方案-2026-09-18基线.md)、[运行时契约](../../current/runtime-contract.md)）；下列条目里的 AgentSlot 指迁移前的宿主侧载体。
...
**H-1–H-4 已实施，旧内核与过渡层已删除，并通过 2026-09-18 集中验证。**

- 运行内核为 `AgentHarness` + `HarnessSlot`：单会话单 lane，输入经持久 inbox（`steer` / `followUp` / `nextRun`），压缩由 Harness 按阈值/手动/溢出调度，陪伴摘要在 `before_compaction` 内生成。
- 会话正文以数据根 `sessions/` 的 JSONL 为真相源（JsonlSessionRepo，commit 事务写入）；`RuntimeQueue`、`AgentSlotRegistry` 与旧压缩调度已删除，不保留双状态机。
- 宿主保留：PermissionKernel 终裁与 Rust 硬边界、Card/人格、RUNTIME_DATA 解析与回复提交、Slash/Preprocessor ingress、MCP 生命周期、NetGuard、Skill 渐进披露。不引入 Node 常驻进程；树导航与 fork 不开放 UI 入口。
- 迁移协议、替换映射、注入点、hook 映射与批次设计见 [AgentHarness 迁移方案基线](../../history/implementation/AgentHarness迁移方案-2026-09-18基线.md)；当前契约见[运行时契约](../../current/runtime-contract.md#请求生命周期)与[当前记忆](../../current/memory.md#压缩提交与恢复)；验证证据与剩余断链见[执行手册](记忆系统重构执行手册.md#当前检查点)。
