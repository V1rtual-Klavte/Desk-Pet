...
# Pi 运行时与工具协议建设方案（2026-09-20 归档）

> **归档说明**：PI-1/PI-2 的 UI、配置字段与工具策略等目标已于 2026-09-20 落地并通过严格门禁；本页保留协议正文、实施订正与验收设计，不再作为当前待办入口。剩余批次与当前进度见[未完成工作与已知缺口](../../plans/active/未完成工作与已知缺口.md)。

本文落实 Pi 能力审查与用户补充：**steer 是聊天感的核心，follow-up 同时保留；只读工具可以并行，写入等有顺序要求的工具串行；每个工具明确声明权限、输出压缩与执行策略。** **§8 的 AgentHarness 迁移（H-1–H-4）已实施并通过 2026-09-18 集中验证**，协议正文归档为 [AgentHarness 迁移方案基线](../../history/implementation/AgentHarness迁移方案-2026-09-18基线.md)，本页 §8 只留索引；**§3–§5 的 UI、配置字段与工具策略等目标已于 2026-09-20 落地并通过严格门禁**（归档时更正：原文"仍未实现"已过时）。

当前事实分别维护在[运行时契约](../../current/runtime-contract.md)、[工具系统](../../current/tool-system.md)、[记忆与压缩](../../current/memory.md)。本文曾是目标协议的主要维护位置，现已归档；当前进度与剩余批次见[未完成工作与已知缺口](../../plans/active/未完成工作与已知缺口.md)。不要求每次任务通读本文：插话读 §3，工具读 §4–5，Pi 接线读 §6，压缩读 §7，Harness 迁移历史读归档基线，实施与配置读 §9–10。
...
以上是会话上下文工作，可以在 P6 之前完成。长期事实抽取、SQLite、中文检索、纠正/遗忘仍按[P6 契约](../../plans/active/记忆系统运行时契约.md)实施，不因压缩算法改变而合并成一个模块。
...
- 迁移协议、替换映射、注入点、hook 映射与批次设计见 [AgentHarness 迁移方案基线](../../history/implementation/AgentHarness迁移方案-2026-09-18基线.md)；当前契约见[运行时契约](../../current/runtime-contract.md#请求生命周期)与[当前记忆](../../current/memory.md#压缩提交与恢复)；验证证据与剩余批次见[未完成工作与已知缺口](../../plans/active/未完成工作与已知缺口.md)。
