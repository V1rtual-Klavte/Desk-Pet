...
> **Archived baseline.** This preserves the full 2026-09-17 active-plan text. Current decisions and status are maintained in [../../plans/active/轻量陪伴运行时与统一内核建设方案.md](../../plans/active/轻量陪伴运行时与统一内核建设方案.md).
...
P4 存量缺口已在本分支收口：摘要结果语义、固定 session、稳定事件顺序、实际 round 身份、工具原文、可验证覆盖边界与请求前预算均已接线。该工作包没有等待 SQLite 或 MemoryKernel；实现与集中验证状态见[执行手册](../implementation/记忆系统重构执行手册-2026-09-17基线.md)。
