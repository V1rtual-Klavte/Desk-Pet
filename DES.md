...
数据流：
  ...
  记忆系统（sessions/ 为唯一会话真相源）:
    memory/MEMORY.md  ← 结构化注册表（## 系统文件 + ## 长期记忆）
    memory/Project.md ← 会话归档指针索引 (→ sessions/session-xxx-主题.md)
    每轮对话 → recordTurn → 实时写入 sessions/<id>-主题.md
    /clear → archiveSession → sessions/<id>-主题.md + 更新 Project.md
    → 每 5 轮自动整理 | 轻量: consolidate() 去重 | 助手: consolidateWithLLM() 矛盾/合并/过期
```
